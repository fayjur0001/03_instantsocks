...
const transactionDummyData: TransactionHistoryData[] = [
  {
    id: "01",
    date: "2025-12-31",
    deposit: "-",
...
const TransactionHistoryTable = ({ searchQuery }: { searchQuery: string }) => {
  ...
  const [toDate_, setToDate] = useState<Date>(toDate("2026-04-30"));

  const [appliedFrom, setAppliedFrom] = useState("2026-01-01");
  const [appliedTo, setAppliedTo] = useState("2026-04-30");
  ...
  const filteredData = useMemo(() => {
    return transactionDummyData.filter((item) => {
      const matchesSearch =
