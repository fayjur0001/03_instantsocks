...
function ProxyTable({ filters, setFilters, setFilter, filteredProxies, selectedProxy, setSelectedProxy, cart, addToCart }: ProxyTableProps) {
    ...
    const columns = useMemo<ColumnDef<ProxyItem, unknown>[]>(
        () => [
            {
                ...
                cell: ({ row }) => (
                    <span className="flex items-center gap-2 font-mono text-[12px]">
                        {/* <span>{toFlagEmoji(row.original.countryCode)}</span> */}
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <span className="text-c-slate-400 text-[12px]">
                        {row.original.state}
                    </span>
                ),
            },
            ...
            },
            {
                ...
                header: "SPEED",
                cell: ({ row }) => (
                    <span className="text-c-slate-300 text-[12px]">
                        ↑ {row.original.speed}
                    </span>
                ),
            },
                    ...
                    return (
                        <div className="flex items-center gap-2 justify-between">
                            ...
                            <button
                                ...
                                className={`w-6 h-6 rounded flex items-center justify-center transition-all ${inCart
        ...
        [filters, setFilter, addToCart, cart]
    );
        ...
        <div className="flex-1 min-w-0 p-4">
            {filteredProxies.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-3 text-c-slate-600">
                    ...
                    <p className="text-[14px] text-c-slate-400">
